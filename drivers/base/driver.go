...
func SaveFileHideList() {
	value := ListToStr(hideFileList)
	model.SaveSetting(model.SettingItem{Key: "excludeFiles", Value: value, Type: "text"})

}
func AddHideFile(hideFile string) {
	hideFileList = append(hideFileList, hideFile)

}
func InitFileHideList() {
	setting, err := model.GetSettingByKey("excludeFiles")
	hideList := make([]string, 0)
	if err != nil {
		model.SaveSetting(model.SettingItem{Key: "excludeFiles", Value: "[/\\\\]node_modules", Type: "text", Description: "support RegExp and multi-line"})
		hideList = append(hideList, "[/\\\\]node_modules")
	} else {
		hideList = strings.Split(setting.Value, "\n")
	}
	hideFileList = hideList
}
func IsHideFile(filePath string) bool {
	for _, hideFileStr := range hideFileList {
		if res, _ := regexp.MatchString(hideFileStr, filePath); res {
			return true
		}
	}
	return false
